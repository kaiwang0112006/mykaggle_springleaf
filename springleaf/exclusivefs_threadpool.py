...
def exclusivefs(train_x, train_y, test_x, test_y):
    ...
    for i in range(long,0,-1):
        print i
        iter = itertools.combinations(findex,3)
        for ilist in iter:
            exclusivelist.append(ilist)
    print "all %d runs" % len(exclusivelist)
    pool = Pool(5)
    pool.map(modelworker,exclusivelist)
...
#         j += 1
    ...
    if auc > 0.8:
        fn = "exclusive/output_%s_%s.thread" % (suffix_i, suffix_j)
        output = open(fn, 'w') 
        output.write("%s\t%s\n" % (str(ilist),str(auc)))
        maxauc = auc
        output.close()
