...
#     indices_test = indices_test.tolist()
    ...
    #modelsing
    print "modelsing"
    gbm = xgb.XGBClassifier(max_depth=3, n_estimators=300, learning_rate=0.05 ,objective='binary:logistic').fit(train_x_trans, train_y)
    ...
    for index, eachline in enumerate(test_pdt):
        fout.write("%s,%s\n" % (str(test_x[index][0]),str(test_pdt[index])))
    fout.close()
